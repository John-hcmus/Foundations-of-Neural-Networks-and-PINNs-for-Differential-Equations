...
đầy đủ cả những chỗ khớp lẫn những chỗ không khớp: trong mười hai hạng mục đối chiếu
ở Mục~\ref{sec:doi-chieu}, bảy khớp trọn vẹn, một khớp về hướng nhưng không về
độ lớn, \emph{hai} buộc phải siết lại cách phát biểu, một chỉ là tương quan chưa
giải thích được, và một cho kết quả trái với kỳ vọng.
...
Mục~\ref{sec:tn1}--\ref{sec:tn9} trình bày chín thí nghiệm.
...
phương pháp: nó giấu đi đúng những chi tiết mà luận văn này lấy làm đối tượng
khảo sát.
...
việc \emph{giải} phương trình, còn luận văn này cần quan sát \emph{cơ chế} của
bộ giải.
...
PINNs suy giảm; với tầng thứ nhất, luận văn ngầm hiểu rằng hằng số ổn định là
...
nghiệm này lặp lại nguyên vẹn TN4 và TN5 trên \emph{năm} hạt giống, cho cả hai
chiến lược trọng số, rồi hỏi một câu duy nhất: \emph{kết luận nào của chương này
sống sót, và kết luận nào chỉ là một lần rút thăm may mắn?}
\end{thinghiem}

\textbf{Thiết lập.} Cấu hình giữ nguyên hoàn toàn so với Mục~\ref{sec:tn4} và
Mục~\ref{sec:tn5}; chỉ hạt giống thay đổi, chạy $s \in \{0,1,2,3,4\}$. Mỗi bài
toán chạy hai chiến lược --- trọng số cố định $\lambda_b = 1$, và ủ trọng số
theo Thuật toán~\ref{alg:annealing} --- thành $20$ lần huấn luyện. Ta báo cáo
\emph{trung vị} cùng \emph{khoảng} $[\min;\max]$ thay vì một giá trị.
...
được ngay trong chính luận văn này.
\end{nhanxet}
...
Áp quy tắc ấy vào Bảng~\ref{tab:doi-chieu}: mọi kết luận ở đó dựa trên chênh
lệch từ một bậc độ lớn trở lên đều đứng vững, trừ hai chỗ đã nêu --- phát biểu
về chiều biến thiên của $\rho_b$ trên bài toán lành mạnh, và quan hệ
$\rho_b$--$\norm{\bW^{(1)}}^2$. Riêng con số $\varepsilon_{L^2}$ của Burgers cần
được đọc như một \emph{khoảng} chứ không phải một giá trị: giá trị
$2{,}84\times10^{-2}$ mà Mục~\ref{sec:tn5} báo cáo nằm trong khoảng năm hạt
giống, nhưng ở phía thuận lợi của nó, và trung vị là $1{,}38\times10^{-1}$.

Bài học phương pháp luận thì rộng hơn một chương: \emph{chi phí của việc lặp năm
hạt giống là hai mươi lần huấn luyện, tức khoảng một giờ máy; cái giá của việc
không lặp là hai kết luận sai.}
\end{nhanxet}
...
$N_r = 2\,500$ điểm phối trí. Mỗi số là trung bình của $20$ lần gọi sau ba lần
làm nóng.

\textbf{Kết quả (a): chi phí một bước.} Tách một bước huấn luyện thành ba mức
chồng lên nhau.

\begin{table}[H]
\centering
\caption[Chi phí một bước huấn luyện PINN]{Chi phí một bước huấn luyện, tách
theo ba mức. Cột cuối là bội số so với lượt xuôi thuần tuý.}
\label{tab:tn9-buoc}
\small
\begin{tabular}{@{}l r r@{}}
\toprule
\textbf{Mức} & \textbf{Thời gian} & \textbf{Bội số} \\
\midrule
Lượt xuôi thuần tuý $u_{\btheta}(X)$        & $2{,}81$ ms  & $1{,}00$ \\
$+$ đạo hàm bậc hai theo $x$ (tính $J_r$)   & $11{,}50$ ms & $4{,}10$ \\
$+$ gradient theo tham số (bước đầy đủ)     & $30{,}35$ ms & $10{,}81$ \\
\bottomrule
\end{tabular}
\end{table}

Hai dự báo lý thuyết đều khớp.

\emph{Nhận xét~\ref{nx:chi-phi-bac-cao} dự báo bốn đến sáu lần; đo được
$4{,}10$.} Đây đúng là đại lượng mà nhận xét nói tới: tỉ số giữa đồ thị có đạo
hàm bậc hai và đồ thị xuôi thuần tuý, cho phương trình bậc hai.

\emph{Mệnh đề~\ref{md:chi-phi-bp} dự báo $c \le 3$; đo được $c = 2{,}64$.}
Hằng số $c$ của mệnh đề là tỉ số giữa chi phí lấy $\nabla_{\btheta}J$ và chi phí
đánh giá $J$ --- ở đây là $30{,}35 / 11{,}50 = 2{,}64$, chứ không phải bội số
$10{,}81$ ở cột cuối. Lấy riêng lượt ngược thì hết $18{,}85$ ms, tức $1{,}64$
lần đồ thị mà nó phải đi ngược. Con số $10{,}81$ là \emph{tích} của hai hiệu
ứng: đạo hàm bậc cao kéo dài đồ thị $4{,}10$ lần, rồi lan truyền ngược nhân
tiếp $2{,}64$ lần. Đây là dạng định lượng của điều
Nhận xét~\ref{nx:chi-phi-bac-cao} phát biểu định tính --- một bước PINN đắt hơn
hẳn một bước hồi quy cùng kích thước mạng.
...
vòng L-BFGS trong $183{,}6$ giây, đạt $\varepsilon_{L^2} = 8{,}19\times10^{-2}$.
Ta hỏi: bộ giải sai phân hữu hạn của Mục~\ref{sec:tn5} cần bao lâu để
đạt \emph{cùng} mức sai số ấy?
...
\begin{tabular}{@{}l r r@{}}
\toprule
\textbf{Phương pháp} & \textbf{Thời gian} & $\varepsilon_{L^2}$ \\
\midrule
PINN, $6\,000$ Adam $+\ 800$ L-BFGS   & $183{,}6$ s & $8{,}19\times10^{-2}$ \\
Sai phân hữu hạn, $N_x = 63$          & $0{,}97$ s  & $7{,}24\times10^{-2}$ \\
\midrule
\multicolumn{3}{@{}l@{}}{\emph{PINN chậm hơn $189$ lần ở cùng mức sai số.}} \\
\midrule
Nghiệm tham chiếu, $N_x = 2\,047$     & $2{,}13$ s  & --- (chuẩn) \\
\bottomrule
\end{tabular}
\end{table}

Hàng cuối bảng đáng dừng lại: chính nghiệm tham chiếu --- lời giải chính xác
nhất của bài toán Burgers trong toàn chương này, cái chuẩn dùng để chấm điểm mọi
kết quả PINN --- được tính trong $2{,}13$ giây, tức nhanh hơn $86$ lần so với
thời gian PINN cần để đạt sai số $8\%$.

Hơn nữa, tỉ số $189$ là ước lượng \textbf{có lợi cho PINN}. Bộ giải sai phân
dùng bước thời gian cố định $\Delta t = 5\times10^{-5}$, chọn cho lưới
$N_x = 2\,047$ để thoả điều kiện ổn định; với $N_x = 63$ thì bước ấy nhỏ hơn
mức cần thiết hàng bậc, nên $0{,}97$ giây còn xa mới là chi phí tối thiểu. Ta
không tinh chỉnh xuống nữa, vì con số đã đủ nói lên điều cần nói.

\begin{nhanxet}[Đọc kết quả này cho đúng: PINN thua ở đâu và vì sao không mâu
thuẫn]
\label{nx:doc-tn9}
Kết quả (b) là kết quả bất lợi rõ rệt nhất cho PINNs trong toàn luận văn, và nó
...
bị phủ nhận: (i) chi phí lưới tăng theo $\mathcal{O}(N^d)$, nên tỉ số $189$ ở
$d=1$ có thể đảo chiều khi $d$ lớn --- đây chính là nội dung
...
trên lưới --- tuy ở mức sai số $8\%$ thì lợi thế này mỏng, vì nội suy lưới sai
...
Kết luận trung thực là: \emph{trên lớp bài toán mà luận văn này thực nghiệm,
PINNs đắt hơn hai bậc so với phương pháp cổ điển ở cùng độ chính xác; giá trị
...
Trong mười hai hạng mục: sáu khớp trọn vẹn, một khớp về hướng nhưng không rút
được kết luận định lượng, \emph{ba} cần siết lại cách phát biểu, một không tái
lập được trên năm hạt giống, và một cho kết quả tiêu cực nhưng đã truy được
nguyên nhân.

Mức khớp này ủng hộ khung lý thuyết của Chương~\ref{chap:pinn} một cách
\emph{có điều kiện}, và điều kiện ấy hiện ra rất rõ khi xếp mười hai hạng mục
theo \emph{nguồn gốc} của phát biểu thay vì theo kết quả.

\emph{Năm trong sáu hạng mục khớp trọn vẹn tương ứng với một mệnh đề đã được
chứng minh}: Hệ quả~\ref{hq:pinn-hong}, Mệnh đề~\ref{md:phat-bien-phat},
Mệnh đề~\ref{md:san-nhieu}, Mệnh đề~\ref{md:chan-on-dinh} và
Mệnh đề~\ref{md:chi-phi-bp}. Hạng mục thứ sáu dựa trên
Nhận xét~\ref{nx:chi-phi-bac-cao}, một ước lượng bậc độ lớn --- và nó khớp đúng
ở mức mà một ước lượng bậc độ lớn có quyền đòi hỏi.

\emph{Ngược lại, không một hạng mục có vấn đề nào tương ứng với một mệnh đề đã
chứng minh.} Cả năm đều đến từ một trong ba kiểu suy luận yếu hơn. Kiểu thứ
nhất, \emph{ghép hai kết quả rời rạc}: giả thuyết \eqref{eq:pho-hieu-dung} nhân
$\abs{P}^2$ với $\widehat{\varkappa}$, và kết luận ở Mục~\ref{subsec:he-qua-pinn}
ngầm giả định thừa số nào thắng. Kiểu thứ hai, \emph{đọc một chặn trên như thể
nó là cơ chế đang vận hành}: tầng thứ nhất của Bảng~\ref{tab:ba-nguyen-nhan}
(Nhận xét~\ref{nx:doc-tn7}). Kiểu thứ ba, \emph{khái quát hoá từ một lần chạy}:
quan hệ $\rho_b$--$\norm{\bW^{(1)}}^2$ không có chứng minh nào cả và không sống
sót qua năm hạt giống (Nhận xét~\ref{nx:tuong-quan-hong}), còn phát biểu về
chiều biến thiên của $\rho_b$ trên bài toán lành mạnh chỉ đúng ở $3/5$ hạt
giống (Mục~\ref{sec:tn8}).
...
\item \emph{Chỉ hai trong năm bài toán được lặp trên nhiều hạt giống.} Nguyên tắc
ở Nhận xét~\ref{nx:doi-xung-hoan-vi} --- với ít nhất $2^{n_1}n_1!$ cực tiểu tương
đương, một kết quả tốt từ một hạt giống may mắn không phải bằng chứng về phương
pháp --- được thực hiện ở Mục~\ref{sec:tn8} cho TN4 và TN5, và ở
Mục~\ref{sec:tn7} cho phép quét $\nu$ (ba hạt giống). Các mục còn lại thì không:
TN1 và TN2 dùng lưới tất định nên vấn đề nhẹ hơn, nhưng \emph{TN3 dùng lấy mẫu
ngẫu nhiên và vẫn chỉ có một hạt giống}. Hệ số $161$ lần của nhúng đặc trưng
Fourier ở Mục~\ref{sec:tn3} vì vậy chưa có khoảng tin cậy nào, và theo ngưỡng đo
được ở Nhận xét~\ref{nx:doc-tn8} thì một hệ số $161$ lần đủ lớn để không thể là
nhiễu hạt giống, nhưng \emph{giá trị} $161$ thì không nên được trích dẫn như một
con số. Khắc phục là chạy nốt TN3 trên năm hạt giống, đúng quy trình của
Mục~\ref{sec:tn8}.
...
\item \emph{Cách sinh mẫu ngẫu nhiên không được đặc tả.} Ba thí nghiệm
TN3, TN4, TN5 dùng điểm phối trí lấy mẫu ngẫu nhiên, nhưng luận văn chỉ ghi
\emph{số lượng} điểm chứ không ghi bộ sinh số ngẫu nhiên và thứ tự tiêu thụ nó.
Hệ quả đo được: khi cài đặt lại từ chính mô tả trong luận văn
(Phụ lục~\ref{app:code}), ba thí nghiệm ấy tái lập \emph{kết luận định tính} một
cách trọn vẹn --- thứ tự học đơn điệu ở TN3, $\rho_b$ giảm ở TN4 và tăng ở TN5,
ủ trọng số phản tác dụng ở cả hai --- nhưng các \emph{con số} lệch từ hai đến
chín lần. Ngược lại, TN1, TN2, TN6 dùng lưới tất định và nghiệm tham chiếu sai
phân hữu hạn của TN5 cũng tất định; cả bốn tái lập tới ba hoặc bốn chữ số có
nghĩa. Bài học: với một thí nghiệm ngẫu nhiên, \emph{số lượng} điểm không phải
là đặc tả đủ.
...
epsilon máy sinh ra ở đó được quỹ đạo tối ưu hoá khuếch đại dần. Điều này không
mâu thuẫn với việc dùng \texttt{float64} --- độ chính xác kép vẫn cần thiết theo
lập luận ở Mục~\ref{ssec:ad-pytorch} --- nhưng nó có nghĩa là tuyên bố ``tái lập
được'' chỉ chặt chẽ khi cố định \emph{cả} hạt giống, \emph{cả} số luồng và
\emph{cả} phiên bản thư viện.
...
giá của việc đó: chậm hơn $189$ lần ở cùng mức sai số. Hệ quả cho cách đọc
...
cho con số bất lợi nhất của luận văn: PINN chậm hơn sai phân hữu hạn $189$ lần ở
cùng mức sai số, trên đúng lớp bài toán mà cả ba động lực của
