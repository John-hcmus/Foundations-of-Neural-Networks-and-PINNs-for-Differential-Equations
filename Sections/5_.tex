...
đầy đủ cả những chỗ khớp lẫn những chỗ không khớp: trong tám hạng mục đối chiếu ở
Mục~\ref{sec:doi-chieu}, năm khớp trọn vẹn, một khớp về hướng nhưng không về độ
lớn, một buộc phải siết lại cách phát biểu, và một cho kết quả trái với kỳ vọng.
...
Mục~\ref{sec:tn1}--\ref{sec:tn5} trình bày năm thí nghiệm.
...
Hàm ở Mã nguồn~\ref{lst:grad} là hiện thực trực tiếp của các công thức truy hồi
...
Ba lựa chọn trong Mã nguồn~\ref{lst:fnn} tương ứng trực tiếp với ba kết luận của
Chương~\ref{chap:fnn}. Khởi tạo Xavier là \eqref{eq:xavier} với độ chệch bằng
$\bm{0}$, đúng quy ước (A2) của Mục~\ref{subsec:phuong-sai}. Vòng lặp
...
Quy trình hai giai đoạn ở Mã nguồn~\ref{lst:train} hiện thực đúng
...
được nhấn mạnh, vì Thí nghiệm~\ref{sec:tn4} sẽ cho thấy một bài toán có
...
Khởi tạo & Xavier chuẩn tắc cho trọng số, $\bb^{(\ell)} = \bm{0}$ \\
...
\begin{table}[H]
\centering
\caption{Thứ tự học các mode tần số. Dấu ``---'' nghĩa là không đạt ngưỡng trong
$8\,000$ vòng lặp.}
\label{tab:tn3}
\small
\begin{tabular}{@{}lrrrr@{}}
...
&\partial_t u = \alpha\, \partial_{xx} u,
  && (x,t) \in (0,1)\times(0,1], \ \ \alpha = 0{,}1,\\
&u(x,0) = \sin(\pi x), \quad u(0,t) = u(1,t) = 0,
\end{aligned}
\right.
\label{eq:heat}
\end{equation}
với nghiệm đúng $u^{\star}(x,t) = \sin(\pi x)\,e^{-\alpha\pi^2 t}$. Ta dùng
...
$\alpha = 0{,}1$ không nhỏ, nên theo Mệnh đề~\ref{md:chan-suy-bien} hằng số ổn
...
vi phạm. Nói cách khác, \textbf{bài toán khuếch tán với $\alpha = 0{,}1$ không
...
báo gốc khuyến nghị $\alpha = 0{,}1$. Ta chạy lại với cả hai giá trị. Hai dòng cuối
...
\begin{equation}
\norm{\nabla_{\btheta} J_i(\btheta)}_{1}
 \;\le\; 2\,G_i\,\sqrt{P\,N_i}\;\sqrt{J_i(\btheta)} ,
...
\hat{\lambda}_i(\btheta)
 \;\ge\; \frac{\sqrt{P}\;\norm{\nabla_{\btheta}J_r(\btheta)}_{\infty}}
              {2G_i\sqrt{N_i}}\cdot\frac{1}{\sqrt{J_i(\btheta)}}
...
Với phạm vi khoá luận này, khuyến nghị là dùng trọng số cố định kèm dò tìm
...
& \textbf{Có chế độ hỏng} (Bổ đề~\ref{bd:phan-hoi-duong}) \\
\bottomrule
\end{tabular}
\end{table}
 
Trong tám hạng mục: bốn khớp trọn vẹn, một khớp về hướng nhưng không rút được kết
luận định lượng, một cần siết lại cách phát biểu, một chỉ là tương quan chưa giải
thích được, và một cho kết quả tiêu cực nhưng đã truy được nguyên nhân.
 
Mức khớp này ủng hộ khung lý thuyết của Chương~\ref{chap:pinn} một cách
\emph{có điều kiện}, và điều kiện ấy đáng được nêu rõ: mọi hạng mục khớp trọn vẹn
đều tương ứng với một mệnh đề \emph{đã được chứng minh}
(Hệ quả~\ref{hq:pinn-hong}, Mệnh đề~\ref{md:phat-bien-phat},
Mệnh đề~\ref{md:san-nhieu}), còn cả ba hạng mục có vấn đề đều tương ứng với những
chỗ luận văn dùng \emph{suy luận tiệm cận} hoặc \emph{ghép hai kết quả rời rạc}:
...
Riêng với Thí nghiệm~\ref{sec:tn5}, nơi sai số tập trung vào một dải hẹp quanh
...
Burgers có thêm một tầng sai số so với Thí nghiệm~\ref{sec:tn4}. Thêm vào đó,
Nhận xét~\ref{nx:hai-nghiem-tham-chieu} đã nêu việc hai chương hiện dùng hai nghiệm
tham chiếu khác nhau.
 
\item \emph{Chưa kiểm chứng chặn ổn định.} Mệnh đề~\ref{md:chan-on-dinh} và
Mệnh đề~\ref{md:chan-suy-bien} cho quan hệ định lượng giữa
$\norm{r_{\btheta}}_{L^2}$ và $\norm{e_{\btheta}}_{L^2}$, và cả hai đều kiểm chứng
được trực tiếp trên bài toán Poisson của Thí nghiệm~\ref{sec:tn1} bằng cách đo
đồng thời hai chuẩn trên lưới đánh giá. Thí nghiệm ấy chưa được thực hiện; đó là
bổ sung đơn giản và có giá trị nhất cho chương này.
\end{enumerate}
