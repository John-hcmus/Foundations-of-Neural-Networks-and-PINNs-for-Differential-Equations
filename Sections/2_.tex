...
luận văn: \emph{mạng nơ-ron truyền thẳng} (feedforward neural network, FNN), còn
...
đại lượng này đóng hai vai trò hoàn toàn khác nhau trong luận văn.
...
$\bm{\delta}^{(\ell)} = \partial J/\partial\bz^{(\ell)}$ & sai số lan truyền
  ngược theo tham số \\
$N$, $N_r$, $N_b$ & cỡ lô; số điểm cấu tứ trong miền; số điểm trên biên \\
...
Luận văn dùng nó nhất quán, bởi nghiệm của một phương trình vi phân nói chung
...
gánh nặng ký hiệu, và vì phần thực nghiệm của luận văn không dùng kiến trúc pha
trộn hàm kích hoạt.
...
số -- con số được xác nhận bằng phép đếm trực tiếp trong thí nghiệm TN1.}
...
điểm -- trong bài toán phương trình vi phân, đó là tập điểm cấu tứ. Đặt
...
$N$: tăng gấp đôi số điểm cấu tứ làm tăng gấp đôi chi phí, không hơn. Thứ hai,
...
\Net'_{\btheta}(0) = \bW^{(2)}\bD^{(1)}\bW^{(1)}
 ...
 = 2 - 1{,}259923 = 0{,}740077 .
\]
Đối chiếu trực tiếp: $\Net'(x) = 2\operatorname{sech}^2 x - 3\operatorname{sech}^2(1-x)$, tại $x=0$ cho $2 - 3\cdot 0{,}419974 = 0{,}740077$. Trùng khớp.
...
\emph{số chiều của miền không gian--thời gian}, chứ không theo số điểm cấu tứ
...
Bruno. Điều cần ghi nhớ cho luận văn là dạng tổng quát của Hệ
...
vì chỉ chỉ ra nghiệm tối ưu, ta thiết lập một \emph{chặn dưới tuyệt đối} cho
...
cho luận văn: hàm $\sigma(t) = \sin t$ dùng trong một số kiến trúc PINN không
...
Đối với luận văn này, dạng $C^0$ ở Định lý~\ref{dl:xap-xi} là \emph{chưa đủ}: nó
...
nào trong luận văn cho phép ghép trực tiếp tốc độ của Barron với chuẩn cần thiết
...
vấn đề kinh nghiệm. Trong bài toán phương trình vi phân, phần lớn lựa chọn bị
...
bằng $\pi/2$, và
\begin{equation}
\big|\sigma^{(k)}(0)\big| \;\lesssim\; k!\left(\frac{2}{\pi}\right)^{k}
\qquad (k \to \infty).
\label{eq:tanh-tang}
\end{equation}
\end{menhde}
...
dị gần gốc nhất cách gốc $\pi/2$, nên bán kính hội tụ bằng $\pi/2$. Ước lượng
\eqref{eq:tanh-tang} là bất đẳng thức Cauchy cho hệ số Taylor:
$|\sigma^{(k)}(0)|/k! \le M(r)/r^{k}$ với mọi $r < \pi/2$, trong đó
$M(r) = \max_{|z|=r}|\tanh z|$.
\end{proof}
...
tăng theo cấp $k!\,(2/\pi)^k$ chứ không nhanh hơn. Trong thực hành, các phương
trình được xét ở luận văn này có bậc không quá $2$, nên chỉ cần
...
\label{eq:J-residual}
\end{equation}
Giả sử $\sigma = \mathrm{ReLU}$ và các điểm cấu tứ $x_i$ được lấy mẫu từ một
...
Hơn nữa, vị trí các siêu phẳng phân hoạch phụ thuộc liên tục vào $\btheta$; do
$\{x_i\}$ hữu hạn và mỗi $x_i$ nằm trong phần trong của ô chứa nó với khoảng
cách dương tới biên, tồn tại lân cận $U$ đủ nhỏ sao cho với mọi $\btheta' \in U$
các điểm $x_i$ vẫn nằm trong phần trong của ô tương ứng. Trên $U$ ta có
...
\end{equation}
tức gradient suy giảm ít nhất theo cấp số nhân với cơ số
$\sech^{2}(\tau) < 1$.
\end{menhde}
...
Ví dụ số cho thấy mức độ nghiêm trọng: với $\tau = 3$ ta có
...
Thí nghiệm TN3 kiểm chứng \eqref{eq:tanh-d1}--\eqref{eq:tanh-d3} bằng sai phân
...
dung hoà bằng cách lấy trung bình điều hoà của $n_{\ell-1}$ và $n_\ell$:
\begin{equation}
\boxed{\;
...
Dùng trong luận văn & không & \textbf{có} (với $\tanh$) & không \\
\bottomrule
\end{tabular}
\end{table}
 
Bảng~\ref{tab:khoitao} cho thấy lý do luận văn chọn Xavier chứ không phải He
...
\begin{proof}
Khai triển Taylor $\tanh t = t - \tfrac{t^3}{3} + \tfrac{2t^5}{15} -\cdots$,
hội tụ với $|t| < \pi/2$ theo Mệnh đề~\ref{md:tanh-giai-tich}, cho
\[
\tanh^2 t = t^2 - \tfrac{2}{3}t^4 + \mathcal{O}(t^6).
\]
Với $z \sim \mathcal{N}(0,s^2)$ ta có $\E[z^2] = s^2$ và $\E[z^4] = 3s^4$, nên
\[
s_\ell^2 = \E\big[\tanh^2 z^{(\ell)}\big]
 = s^2 - \tfrac{2}{3}\cdot 3s^4 + \mathcal{O}(s^6)
 = s^2 - 2s^4 + \mathcal{O}(s^6),
\]
với $s = s_{\ell-1}$, do $\rho = 1$ kéo theo $\Var(z^{(\ell)}) = s_{\ell-1}^2$
theo \eqref{eq:lan-truyen-phuong-sai}. Đó là \eqref{eq:hieu-chinh}.
...
\begin{table}[H]
\centering
\caption{Kết quả thí nghiệm TN4: mạng $12$ lớp, mỗi lớp $128$ nơ-ron, kích hoạt
...
khởi tạo Xavier (số liệu cùng thí nghiệm TN4).}
\label{tab:dieu-hoa}
...
đối giữ dưới $9\%$ trên toàn bộ dải khảo sát -- mức khớp tốt đối với một công
thức tiệm cận không chứa tham số tự do nào.
...
đã được cập nhật -- điều mà không kết quả nào trong luận văn khẳng định.
\end{nhanxet}
 
\subsection{Khởi tạo độ chệch và lựa chọn của luận văn}
...
suốt phần thực nghiệm của luận văn là: hàm kích hoạt $\tanh$, khởi tạo Xavier
theo \eqref{eq:xavier-uniform}, độ chệch bằng $0$, lớp ra tuyến tính. Cấu hình
...
PINN, số điểm cấu tứ $N$ bị giới hạn bởi bộ nhớ chứ không bởi thời gian tính.
 
Thí nghiệm TN1 xác nhận tính đúng đắn của cài đặt trên kiến trúc $(2,4,4,1)$ với
...
\label{md:gauss-mse}
Chọn $p_{\text{model}}(\bm{y}\mid\bx;\btheta)
 = \mathcal{N}\!\big(\bm{y};\, \Net_{\btheta}(\bx),\,
   \varsigma^{2}\bI_{n_L}\big)$ với $\varsigma > 0$ cố định. Khi đó
\begin{equation}
J(\btheta)
 = \frac{1}{2\varsigma^{2}}\,
   \E_{\hat{p}_{\text{data}}}
...
điểm cực tiểu), và sai số rời rạc hoá (tập điểm cấu tứ hữu hạn). Việc phân tách
...
phương pháp luận thực nghiệm của luận văn phải dựa trên nhiều lần chạy với hạt
...
Chương này đã thiết lập các kết quả nền tảng cho phần còn lại của luận văn, được
...
hoạt $\tanh$, khởi tạo Xavier theo \eqref{eq:xavier-uniform}, độ chệch bằng $0$,
...
luận văn là bắt buộc chứ không phải minh hoạ.
