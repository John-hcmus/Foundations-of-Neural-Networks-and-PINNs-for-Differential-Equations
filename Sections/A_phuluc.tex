...
PyTorch thuần, không dùng thư viện PINNs đóng gói sẵn, và được công bố kèm luận
văn trong thư mục \texttt{code/}. Phụ lục này mô tả cấu trúc và các quy ước cần
...
\texttt{pinns/exp7\_nu\_sweep.py} & Mục~\ref{sec:tn7}, quét $\nu$ trên ba hạt
  giống \\
\texttt{pinns/exp8\_seeds.py} & Mục~\ref{sec:tn8}, lặp lại TN4 và TN5 trên năm
  hạt giống; chạy tiếp được sau khi bị ngắt, vì ghi kết quả sau mỗi hạt giống \\
...
\texttt{pinns/run\_all.py} & Chạy lần lượt cả chín thí nghiệm \\
...
đối chiếu lại với các bảng trong luận văn.
...
Mọi thí nghiệm dùng một hạt giống cố định (\texttt{seed = 0}) và số học
\texttt{float64}, nên kết quả tái lập được trên cùng một phiên bản PyTorch. Cần
đọc điều này kèm hạn chế thứ nhất ở Mục~\ref{sec:hanche}: tái lập được không đồng
nghĩa với có ý nghĩa thống kê, và một hạt giống duy nhất chưa đủ để rút kết luận
so sánh giữa hai phác đồ huấn luyện.
