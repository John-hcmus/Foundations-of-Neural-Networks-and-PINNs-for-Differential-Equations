...
hàm, và -- điểm quyết định đối với luận văn -- áp dụng được \emph{đệ quy lên
...
\item $\varrho(\hat{\bm{y}}, \bm{y})$ -- hàm mất mát \emph{tại một điểm}. Ta
      ...
      dành riêng cho toán tử vi phân trong toàn bộ luận văn.
...
sự~\cite{goodfellow2016}, tách bạch \emph{cấu trúc} của phép tính đạo hàm khỏi
\emph{nội dung} cụ thể của mô hình. Hệ thức truy hồi
...
trở thành công cụ đúng cho bài toán của luận văn: phần dư
...
để đối chiếu, nhưng luận văn không dùng trường hợp này.)
...
\emph{(c) Trường hợp của luận văn.} Với lớp ra tuyến tính và
...
= 0{,}045052,
\qquad
J = \tfrac{1}{2}(0{,}045052 - 0{,}5)^2 = 0{,}103489.
\]
...
đi nhanh chóng trong bối cảnh của luận văn.
...
Đây là mục có ý nghĩa quyết định đối với phần còn lại của luận văn, và là mắt
...
\verb|torch.autograd.grad(u, x, create_graph=True)|, trong đó cờ
\verb|create_graph| báo cho hệ thống rằng bản thân phép tính gradient phải được
...
\lim_{k\to\infty}\E\big[J(\theta_k)\big]
 ...
 \;\xrightarrow[\alpha\to 0]{}\; \frac{\alpha\varsigma^2}{4}.
\label{eq:san-nhieu}
\end{equation}
\end{menhde}
...
nhỏ. Phương pháp \emph{động lượng} khắc phục bằng cách tích luỹ một trung bình
trượt của các gradient quá khứ:
\begin{equation}
...
$\bm{v}_k \to -\alpha\bm{g}\,(-1)^k/(1+\beta)$, tức biên độ bị \emph{thu nhỏ} đi
thừa số $1/(1+\beta)$.
\end{bode}
...
cho \eqref{eq:momentum-khuech-dai}. Với $\bm{g}_{k-j} = (-1)^{k-j}\bm{g}$, tổng
trở thành $(-1)^k\sum_j (-\beta)^{j} \to (-1)^k/(1+\beta)$.
\end{proof}
...
tổng tích luỹ bằng trung bình trượt hàm mũ, khắc phục nhược điểm đó. Adam kết hợp
...
$\|\btheta_{k+1}-\btheta^{*}\| / \|\btheta_k - \btheta^{*}\| \to 0$. L-BFGS với
...
$\btheta_0 = (-1{,}2;\,1)$ và cùng nghiệm $\btheta^{*}=(1;\,1)$. (a) Quỹ đạo
...
một tập điểm cấu tứ \emph{cố định}, nên $J(\btheta)$ là hàm tất định và
...
\label{alg:two-stage}
\begin{algorithmic}[1]
\Require Tham số khởi tạo $\btheta_0$ theo Xavier \eqref{eq:xavier-uniform}, số
...
chiến lược này trên hai bài toán phương trình vi phân thường sẽ được trình bày ở
...
Chương này đã thiết lập bộ công cụ tính toán cho phần còn lại của luận văn; các
