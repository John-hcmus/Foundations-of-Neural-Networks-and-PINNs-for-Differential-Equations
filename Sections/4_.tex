...
trong~\cite{raissi2019}, bởi $\lambda$ trong luận văn này được dành cho các trọng
...
trong luận văn. Với phương trình Burgers một chiều có độ nhớt $\nu$,
...
sai số tương đối theo chuẩn vô cùng. Không kết quả nào trong luận văn cho phép
...
cách tự động. Luận văn dùng ràng buộc mềm cho các thí nghiệm chính và ràng buộc
...
đầu, điều kiện biên và phương trình. Đây là trường hợp của hai thực nghiệm ở
Chương~\ref{chap:thucnghiem}.
...
riêng. Đây là ưu thế nổi bật so với các phương pháp bài toán ngược cổ điển, vốn
đòi hỏi giải lặp bài toán thuận nhiều lần.
...
không gian--thời gian, với mạng $9$ lớp ẩn, $20$ nơ-ron mỗi lớp. Kết quả nhận
dạng: với dữ liệu sạch, $\zeta_1 = 0{,}99915$ và
...
Kết luận thứ hai: sai số của $\zeta_2$ luôn lớn hơn sai số của $\zeta_1$ khoảng
một bậc, và điều này không ngẫu nhiên -- $\zeta_2$ nhân với $\partial_{xx}u$, đại
lượng khó ước lượng nhất từ dữ liệu, đúng theo tinh thần
Nhận xét~\ref{nx:fd-pde}. Dè dặt: các bảng khảo sát độ nhạy
...
\State Khởi tạo $\btheta_{0}$ theo Xavier \eqref{eq:xavier-uniform}; đặt
...
Thứ ba, định lý phát biểu cho ReLU. Mạng $\tanh$ dùng trong luận văn không thuộc
...
dụng trực tiếp cho kiến trúc của luận văn.
\end{nhanxet}
...
Thứ nhất, khởi tạo Xavier \eqref{eq:xavier-uniform} cho chuẩn trọng số nhỏ theo
...
\end{equation}
nên với các $k$ lớn, $\abs{\widetilde{r}_{\btheta}(k)} \sim
\norm{k}^{m}\,\abs{\widetilde{e}_{\btheta}(k)}$, sai khác một hằng số bằng modul
của hệ số dẫn đầu.
\end{menhde}
...
\eqref{eq:residual-amplification}. Khi $\norm{k}\to\infty$, ký hiệu chính (các số
hạng bậc $m$) trội hơn mọi số hạng bậc thấp hơn.
\end{proof}
...
Mệnh đề~\ref{md:khuech-dai} như phân tích ký hiệu chính, không như một đẳng thức
cho bài toán đầy đủ.
...
dao động tần số cao. Ta sẽ kiểm chứng định lượng nhận định này qua hai bài toán
ODE ở Chương~\ref{chap:thucnghiem}.
